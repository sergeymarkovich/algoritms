...
class NaiveBinTree {
    ...
    struct mTreeNode{
        ...
        mTreeNode* mRight;
        mTreeNode* mParent;
        mTreeNode(T data, mTreeNode* left, mTreeNode* right, mTreeNode* parent):
            mData(data), mLeft(left), mRight(right), mParent(parent) {};
    };
    mTreeNode* head;
...
void NaiveBinTree<T>::InsertData(const T& data) {
    ...
    if (pointer == nullptr) {
        pointer = new mTreeNode(data, nullptr, nullptr, nullptr);
        head = pointer;
        return;
    }
    ...
    while (pointer != nullptr) {
        if (mCompare(data, pointer->mData)) {
            if (pointer->mLeft != nullptr) {
                ...
                mTreeNode* q = new mTreeNode(data, nullptr, nullptr, pointer);
                pointer->mLeft = q;
                return;
            }
...
template<class T>
Treap<T>::~Treap() {
    std::list<mTreapNode*> mList;
    mList.push_front(head);

    while (!mList.empty()) {
        mTreapNode* tmp = mList.front();
        mList.pop_front();
        if (tmp->mRight != nullptr) {
            mList.push_front(tmp->mRight);
        }
