...
private:
    ...
    // удаление элемента
    AvlTreeNode* find_remove_min(AvlTreeNode* a, AvlTreeNode*& min);
...
template<class T>
typename AvlTree<T>::AvlTreeNode* AvlTree<T>::find_remove_min(AvlTreeNode* a, AvlTreeNode*& min) {
    if (a->mLeft == nullptr) {
        min = a;
        return a->mRight;
    }
    a->mLeft = find_remove_min(a->mLeft, min);
    return Balance(a);
}
...
typename AvlTree<T>::AvlTreeNode* AvlTree<T>::deleteData(AvlTreeNode* a, const T& data) {
    ...
    if (data < a->mData) {
        ...
        AvlTreeNode* temp = find_remove_min(r, min);
        min->mLeft = l;
        min->mRight = temp;
        return Balance(min);
    }
